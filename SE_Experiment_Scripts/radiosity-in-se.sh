...
build/$1/gem5.opt --outdir=radiosity-se-$1 configs/example/se.py --mem-size="4096MB" --num-cpus=64 --num-dirs=64 --network=simple --topology=Mesh_XY --mesh-rows=8 --cpu-type TimingSimpleCPU --ruby --cmd="$P_PATH/ext/splash2x/apps/radiosity/inst/amd64-linux.gcc/bin/radiosity" --options="$SIMSMALL -p 64"
