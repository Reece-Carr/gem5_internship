...
../build/$1/gem5.opt --outdir=../Experiment_Results/$1/streamcluster-se-$1 ../configs/example/se.py --num-cpus=64 --num-dirs=64 --network=simple --topology=Mesh_XY --mesh-rows=8 --cpu-type TimingSimpleCPU --ruby --cmd="$P_PATH/pkgs/kernels/streamcluster/inst/amd64-linux.gcc/bin/streamcluster" --options="$SIMSMALL 64"
